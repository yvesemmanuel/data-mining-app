...
def regular_payments():
    ...
    if user_request == 'POST':
        ...
        #description = str(selected_city)+" "+str(selected_year)
...
def payments_delay():
    ...
    if user_request == 'POST':
        ...
        options_source = get_delay_sources(selected_year, selected_city_num, selected_limit, options_loan_types[selected_loan_type], options_entity_types[selected_entity])
        selected_uo_source = options_source[0] if selected_action in ['apply'] else request.form.get('source')

        _, selected_uo, selected_source = selected_uo_source.split(' + ')

        df = get_pagamentos_atrasados(selected_year, selected_city_num, selected_limit, options_loan_types[selected_loan_type], options_entity_types[selected_entity],selected_uo, selected_source)
